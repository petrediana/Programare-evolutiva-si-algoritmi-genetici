...
while i<=dim
    ...
    sir=unifrnd(0,1,1,n); %generare unui individ cu caracteristici aleatoare
    val=f_obiectiv(valoare,sir,n); %calculare functie fitness
    
    %verificarea fezabilitatii pe baza valorii costului
    [OK,~]=este_fezabil(sir,cost,n,cmax);
    if(OK) %daca este valid, se completeaza matricea parintilor cu respectivul individ
        Pop(i,1:n)=sir;
        Pop(i,n+1)=val;
        i=i+1; %daca solutia a fost acceptata, se trece la urmaorul parinte
    end;
end;
end
